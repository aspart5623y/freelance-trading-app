            ...
            <div class="card card-body border-0 d-block">
                ...
                @if (auth()->user()->account->count() < 1)
                    <p class="alert alert-danger">
                        ...
                        <a href="{{ route('investor.bank') }}" class="text-success">Bank Account</a>, <a href="{{ route('investor.crypto') }}" class="text-success">Crypto wallet</a> and <a href="{{ route('investor.paypal') }}" class="text-success">Paypal</a> .</p>
                @endif

                <form action="{{ route('investor.withdrawal.store') }}" method="post">
                    @csrf
