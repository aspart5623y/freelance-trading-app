    ...
    <div class="row justify-content-center">
        <div class="col-xl-4 col-lg-12 col-md-5">
            <div class="card card-body border-0 mb-4 d-block">
                ...
                <form action="{{ route('trader.fund') }}" id="transfer-form" method="post">
                    @csrf
                    ...
                    @foreach ($transfers as $item)
                        @if ($item->profile_id == auth()->user()->id)
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-sm-2">
                                    ...
                                    <h6 class="text-danger">-${{ number_format($item->amount) }}</h6>
                                </div>
                        ...
                        @else 
                            @if ($item->status == 'successful')
                                ...
                                <div class="row align-items-center">
                                    <div class="col-sm-5">
                                        <small class="text-muted fw-bold">Sender</small>
                                        <h6 class="mb-0">{{ $item->profile->profileable->firstname . ' ' . $item->profile->profileable->lastname }}</h6>
                                        <span class="text-muted">{{ $item->reciever_address }}</span>
                                    </div>
                                
                                    <div class="col-sm-2">
                                        <small class="text-muted fw-bold">Amount</small>
                                        <h6 class="text-success">+${{ number_format($item->amount, 2) }}</h6>
                                    </div>
